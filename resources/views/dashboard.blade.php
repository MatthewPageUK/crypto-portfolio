<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center">
            ...
            <div class="flex-grow">
                <a href="{{ route('addtoken') }}" title="Add a new token" class="flex items-center text-right">
                    <span class="flex-grow">Add Token</span> 
                    <svg xmlns="http://www.w3.org/2000/svg" class="ml-1 text-gray-500 hover:text-gray-800 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        @foreach ($tokens as $token)
                            <div class="w-64 p-6 m-5 bg-white shadow-lg rounded-lg text-center">
                                ...
                                <p class="mt-3 flex items-center text-center">
                                    <a href="{{ route('buy', $token->id) }}" class="mr-2 flex items-center justify-center flex-grow p-1" title="Buy {{ $token->symbol }}">
                                    ...
                                    @if( $token->balance > 0 )
                                        <a href="{{ route('sell', $token->id) }}" class="ml-2 flex items-center justify-center flex-grow p-1" title="Sell {{ $token->symbol }}">
